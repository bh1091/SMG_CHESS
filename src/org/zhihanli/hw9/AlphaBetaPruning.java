...
public class AlphaBetaPruning {
		...
		try {
			for (int i = 0; i < depth; i++) {
				for (MoveScore moveScore : scores) {
					...
					int score = 0;
					score = findMoveScore(changeState(state, move), i,
	...
	private int findMoveScore(State state, int depth, int alpha, int beta,
		...
		for (Move move : possibleMoves) {
			...
			int childScore = findMoveScore(changeState(state, move), depth - 1,
					alpha, beta, timer);
			if (color == null) {
				scoreSum += childScore;
	...
	public State changeState(State state, Move move) {
		State stateCopy = state.copy();
		try {
			stateChanger.makeMove(stateCopy, move);
		} catch (Exception e) {
			System.out.println("In change state: " + state);
