...
public class AlphaBetaPruning {
	...
	static class MoveScore implements Comparable<MoveScore> {
		...
		@Override
		public int compareTo(MoveScore m) {
			return m.score - score; // sort DESC (best score first)
		}
	}

	private ManderHeuristic heuristic;

	public AlphaBetaPruning(ManderHeuristic heuristic) {
	...
	private int findMoveScore(State state, int depth, int alpha, int beta,
		...
		for (Move move : possibleMoves) {
			...
			sc.makeMove(newState, move);
			int childScore = findMoveScore(newState, depth - 1,
					alpha, beta, timer);
			if (color == null) {
				scoreSum += childScore;
