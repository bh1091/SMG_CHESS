...
public class PresenceServlet extends HttpServlet {
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse resp) {
    ...
    try {
      ...
      //String userID = channelToken.substring(0, channelToken.lastIndexOf('-'));
      ...
      if (channelPresence.isConnected()) {
        ...
        /*Set<Key<Match>> matches = player.getMatchesList();
        for (Key<Match> matchKey : matches) {
          Match match = ofy().load().key(matchKey).get();
          Key<Player> opponent = match.getOpponent(playerKey);
          Player otherPlayer = ofy().load().key(opponent).get();
          String msg = "otherdisconnected#"+otherPlayer.getPlayerName();
