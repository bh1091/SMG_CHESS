...
public class AlphaBetaPruning {
  ...
  private static StateChangerImpl stateChanger;
      ...
      for (int i = 0; i < depth; i++) {
        for (MoveScore<Move> moveScore : scores) {
          ...
          try {
            ...
            int score = findMoveScore(copy, i, Integer.MIN_VALUE, Integer.MAX_VALUE, timer);
            if (!isWhite) {
  ...
  /*public static void main(String args[]) {
    ...
    stateChanger.makeMove(state, new Move(new Position(1, 5), new Position(2, 5), null));
    stateChanger.makeMove(state, new Move(new Position(6, 4), new Position(4, 4), null));
    stateChanger.makeMove(state, new Move(new Position(1, 6), new Position(3, 6), null));
    Move move = abp.findBestMove(state, 5, new DateTimer(5000));
