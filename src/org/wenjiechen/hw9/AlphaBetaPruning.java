...
public class AlphaBetaPruning {
              ...
              try {
                      for (int i = 0; i < depth; i++) {
                              for (MoveScore moveScore : scores) {
                                      ...
                                      State newState=stateChange(state, move);
	...
	private int findMoveScore(State state, int depth, int alpha, int beta,
		...
		for (Move move : possibleMoves) {
			count++;
			int childScore = findMoveScore(stateChange(state, move), depth - 1, alpha, beta,timer);
			if (color == null) {
				scoreSum += childScore;
      ...
      public State stateChange(State state,Move move){
