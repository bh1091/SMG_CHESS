...
import org.chenji.hw9.AlphaBetaPruning;
import org.chenji.hw9.Heuristic;
import org.chenji.hw9.Timer;
...
public class Presenter {
  public interface View {
    ...
    boolean isPlayingWithAI();
    
    void clickSound();
    
    void addHistory(String state);
  }
  ...
  private AlphaBetaPruning alphaBetaPruning = new AlphaBetaPruning(new Heuristic());
  ...
  public void setState() {
    ...
    view.addHistory(state.toString());
  }
  ...
  public void AIMove() {
    Move move =  alphaBetaPruning.findBestMove(state, 10, new Timer(2000));
    selectPiece(move.getFrom());
    Piece piece = state.getPiece(selected);
    view.setPiece(selected.getRow(), selected.getCol(), null);
    view.animation(move, piece);
  ...
  public boolean canMakeMove(Move move) {
    return moveCheck.isCanMakeMove(state, move);
  }
}
