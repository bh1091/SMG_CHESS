...
import org.chenji.hw6.Presenter.View;
import org.chenji.hw6.client.GameService;
import org.chenji.hw6.client.GameServiceAsync;
...
public class Graphics extends Composite implements View {
  ...
  public void connect(LoginInfo loginInfo) {
    final Logger logger = Logger.getLogger(Graphics.class.toString());
    gameService.connect(loginInfo, new AsyncCallback<Player>() {
      ...
      public void onSuccess(Player result) {
        player = result;
        playerStatus.setText(player.getPlayerId()
            + ", "
            + player.getColor().toString()
            + "; "
            + (player.isWating() ? "Waiting for other players"
                : "Ready to play"));
        ...
        channel.open(new SocketListener() {
          ...
          public void onOpen() {
            ...
            if (!player.isWating()) {
              gameService.updateState(player.getPlayerId(),
                  new State().toString(), new AsyncCallback<Void>() {
                    @Override
                    public void onFailure(Throwable caught) {
                      Window.alert("Fail!");
                    }
          ...
          @Override
          public void onMessage(String state) {
            ...
            playerStatus.setText(player.getPlayerId()
                ...
                + player.getColor().toString()
                + "; "
                + (player.getColor() == presenter.getTurn() ? "Your turn" : "opponents' turn"));
  ...
  public void updateState(String state) {
    gameService.updateState(player.getPlayerId(), state,
  ...
  private boolean isTurn() {
    return !isWaiting && player != null
        && player.getColor() == presenter.getTurn();
  }
